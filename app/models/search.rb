...
class Search < ActiveRecord::Base
  ...
  def self.find_nearby_places(address, keyword, radius)
   places_address = Addressable::URI.new(
     ...
     :query_values => {
       :key => NEW_KEY,
       :location => Search.lat_lng(address),
       :radius => radius,
       :sensor => false,
       :keyword => keyword
      }
   ...
   def self.find_nearby_places_lat_long(address, subject, radius)
     places_JSON = Search.find_nearby_places(address, subject, radius)
   ...
   def self.find_nearby_places_by_name(address, subject, radius)
     places_JSON = Search.find_nearby_places(address, subject, radius)
